# -*- coding: utf-8 -*-
"""
Created on Mon Sep 21 14:58:51 2020
...
map_district = gpd.read_file(r'npl_admbnda_nd_20190430_shp\npl_admbnda_districts_nd_20190430.shp')
map_district = map_district[['DIST_EN','geometry']]

#Loading map of Nepal with local units
map_nep = gpd.read_file(r'nepal administ\Local Unit\local_unit.shp')
...
#Data Preprocessing
districts = pd.read_csv('csv data\district_list.csv',index_col=0)

covid_cases = pd.read_csv('csv data\sep21_recovered_district.csv')
covid_cases = covid_cases[['DISTRICT_id','Recovered']]


values = covid_cases['DISTRICT_id'].tolist()
values_case = covid_cases['Recovered'].tolist()
new_values = []
for value, d_val in zip(values,values_case):
    new_values.append((district_dict[value], value, d_val ))
    
covid_cases = pd.DataFrame(columns={'DISTRICT','DISTRICT_id','Recovered'}, data=new_values)
covid_cases = covid_cases.set_index('DISTRICT_id')


districts = districts.join(covid_cases['Recovered'], on='DISTRICT_id', rsuffix='Recovered')
districts = districts.reset_index(drop=True)

#filling nan values with 0
districts['Recovered'] = districts['Recovered'].fillna(0)


new_map = new_map[['DISTRICT','geometry']]
#districts = districts[['DISTRICT','Active']]
new_map = new_map.merge(districts, on='DISTRICT')
new_map.plot(column='Recovered',figsize=(10,8),cmap='nipy_spectral',legend=True)
