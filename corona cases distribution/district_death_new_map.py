...
covid_cases = pd.read_csv('csv data\sep21_death_district.csv')
covid_cases = covid_cases[['DISTRICT_id','Death']]


values = covid_cases['DISTRICT_id'].tolist()
values_case = covid_cases['Death'].tolist()
new_values = []
for value, d_val in zip(values,values_case):
    new_values.append((district_dict[value], value, d_val ))
    
covid_cases = pd.DataFrame(columns={'DISTRICT','DISTRICT_id','Death'}, data=new_values)
covid_cases = covid_cases.set_index('DISTRICT_id')


districts = districts.join(covid_cases['Death'], on='DISTRICT_id', rsuffix='Death')
districts = districts.reset_index(drop=True)

#filling nan values with 0
districts['Death'] = districts['Death'].fillna(0)


new_map = new_map[['DISTRICT','geometry']]
#districts = districts[['DISTRICT','Active']]
new_map = new_map.merge(districts, on='DISTRICT')
new_map.plot(column='Death',figsize=(10,8),cmap='nipy_spectral',legend=True)
